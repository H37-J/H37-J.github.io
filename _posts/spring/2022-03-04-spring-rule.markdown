...
Client는 그저 인터페이스만을 의존하여 각 객체를 의존 할 수 있다.
...
이는 OrderAfterService 클래스 하나가 너무 많은 로직에 대한 책임이 있다. 또 나중에 메일에 대한 서비스가 변경 된다면 
포인트 적립에 대한 로직까지 영향을 끼칠 수 있다. 이를 아래와 같이 변경한다
...
어떤 인터페이스를 구현하는 클래스가 있을 때, 그 클래스가 인터페이스의 모든 메서드를 구현할 필요가 있는 인터페이스만 상속 받도록 해야한다. 만약 그렇지 않다면 인터페이스를 더 세분화 하여 나눈다.
...
그런데 이렇게 구현하면 NormarUser 클래스에서는 필요가 없는 AdminAct까지 구현을 해줘야 한다.
